"""
Ensemble Methods using only PhoBERT-base and PhoBERT-large
Combines predictions from the two best performing models.
"""
...
def evaluate_ensemble(preds, labels, ensemble_name):
    ...
    results = {
        ...
        "base_models": ["phobert_large", "phobert_base"],
    ...
    filename = ensemble_name.lower().replace(' ', '_').replace('-', '_')
...
def main():
    print("="*60)
    print("PhoBERT-only Ensemble (Base + Large)")
    print("="*60)
    ...
    # Only use PhoBERT-base and PhoBERT-large
    phobert_models = [
    ...
    for model_name, pretrained_name in phobert_models:
        model_dir = os.path.join(RESULTS_DIR, model_name)
        if os.path.exists(model_dir):
            try:
                ...
                print(f"  {model_name}: Accuracy = {accuracy_score(labels, preds):.4f}")
    ...
    print(f"\nUsing models: {list(all_preds.keys())}")
    
    # Ensemble 1: Majority Voting
    print("\n" + "-"*40)
    preds_list = [all_preds["phobert_large"], all_preds["phobert_base"]]
    voting_preds = majority_voting(preds_list)
    evaluate_ensemble(voting_preds, labels, "PhoBERT_Majority_Voting")
    
    # Ensemble 2: Soft Voting (equal weights)
    probs_list = [all_probs["phobert_large"], all_probs["phobert_base"]]
    soft_preds, _ = soft_voting(probs_list)
    evaluate_ensemble(soft_preds, labels, "PhoBERT_Soft_Voting")
    
    # Ensemble 3: Weighted Voting (favor large model)
    # PhoBERT-large: 0.6, PhoBERT-base: 0.4
    weights = [0.6, 0.4]
    weighted_preds, _ = soft_voting(probs_list, weights=weights)
    evaluate_ensemble(weighted_preds, labels, "PhoBERT_Weighted_0.6_0.4")
    
    # Ensemble 4: Weighted Voting (favor base model - since it has better F1)
    # PhoBERT-large: 0.4, PhoBERT-base: 0.6
    weights = [0.4, 0.6]
    weighted_preds2, _ = soft_voting(probs_list, weights=weights)
    evaluate_ensemble(weighted_preds2, labels, "PhoBERT_Weighted_0.4_0.6")
    
    # Ensemble 5: Equal weights
    weights = [0.5, 0.5]
    equal_preds, _ = soft_voting(probs_list, weights=weights)
    evaluate_ensemble(equal_preds, labels, "PhoBERT_Equal_Voting")
    
    print("\n" + "="*60)
    print("PhoBERT Ensemble results saved to: results/ensembles/")
    print("="*60)
